...
const scriptStartTag = '<style>'; // since the style tag follows the detail modal
      ...
      <div class="modal-content rounded-4 border-0 overflow-hidden shadow-lg">
        <div class="modal-header border-0 bg-primary text-white p-4 align-items-center">
          ...
          <div class="d-flex align-items-center gap-2">
            <button type="button" class="btn btn-white btn-sm rounded-pill text-primary fw-bold px-3 hover-lift" id="btnEditTarget">
            ...
            <div class="col-lg-7 p-4 p-md-5 d-flex flex-column h-100" style="max-height: 80vh; overflow-y: auto;">
              ...
              <div class="card border-0 shadow-sm rounded-4 mb-4 flex-shrink-0">
                <div class="card-body p-4">
                  ...
                  <button type="button" class="btn btn-primary w-100 rounded-pill py-3 fw-bold shadow-sm hover-lift" id="btnSaveTransaction">
                ...
                <div class="card-body p-4 d-flex flex-column h-100">
                  ...
                  <div class="d-flex justify-content-between align-items-center mt-3 pt-3 border-top" id="targetTxPagination">
                    ...
                    <div class="d-flex gap-2">
                      <button class="btn btn-sm btn-light rounded-pill px-3 fw-semibold" id="btnPrevTxPage" disabled>Sebelumnya</button>
                      <button class="btn btn-sm btn-light rounded-pill px-3 fw-semibold" id="btnNextTxPage" disabled>Selanjutnya</button>
                    </div>
                  </div>
                </div>
              </div>
  ...
  <div class="modal fade" id="editTargetModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content rounded-4 border-0 shadow-lg p-4">
        ...
        <button type="button" class="btn btn-primary rounded-pill w-100 fw-bold py-2 hover-lift" id="btnUpdateTarget">Simpan Perubahan</button>
        
        <hr class="my-4">
        <button type="button" class="btn btn-outline-danger rounded-pill w-100 fw-bold py-2" id="btnDeleteTarget">
          <i class="ti ti-trash me-2"></i>Hapus Target Secara Permanen
        </button>
      </div>
    </div>
  </div>
  ...
  <div class="modal fade" id="editTransactionModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-sm">
      <div class="modal-content rounded-4 border-0 shadow-lg p-4">
        ...
        <button type="button" class="btn btn-light rounded-pill w-100 fw-bold py-2 text-danger" id="btnDeleteTx">Hapus Riwayat</button>
      </div>
    </div>
  </div>
...
const newHtml = html.substring(0, startIndex) + newModalHTML + html.substring(endIndex);

fs.writeFileSync('docs/savings.html', newHtml);
console.log('savings.html updated');
`;

fs.writeFileSync('docs/update_savings_detail_html.js', code);
console.log('update_savings_detail_html.js written');
