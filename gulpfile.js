...
const paths = {
  ...
  dist: {
    basedist: './docs',
    js: './docs/assets/js',
    vendorJs: './docs/assets/js/vendors',
...
// Copy app-level JS (storage.js, app.js) to dist
function copyAppJs(callback) {
  return src(['./src/assets/js/*.js', '!./src/assets/js/vendors/**'], { base: './src' })
    .pipe(gulpTerser())
    .pipe(dest(paths.dist.basedist));
  callback();
}
...
// Build Task for Dist
exports.build = series(parallel(cleanDist), html, images, fonts, vendorJs, copyAppJs, copyLibs, cleanTemp);
